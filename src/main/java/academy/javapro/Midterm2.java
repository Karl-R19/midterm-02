...
public class Midterm2 {
	 ...
	 */
	public static <T, S, R> List<R> mergeCollections(Collection<T> firstCollection, Collection<S> secondCollection,
			BiFunction<T, S, R> mergeFunction) {

		// Validate that both collections have the same size
		if (firstCollection.size() != secondCollection.size()) {
			throw new IllegalArgumentException(
					"Collections must have the same size for element-wise merging. " + "First collection size: "
							+ firstCollection.size() + ", Second collection size: " + secondCollection.size());
		}
		...
		// Loop through the arrays and apply the merge function
		...
		// Return the list containing the merged results
		return result;
	 ...
	 */
	public static <T, S, R> List<R> zipCollections(Collection<T> firstCollection, Collection<S> secondCollection,
			BiFunction<T, S, R> mergeFunction) {

		// Initialize 'result' with initial capacity set to the minimum size of the two
		// input collections
		List<R> result = new ArrayList<>(Math.min(firstCollection.size(), secondCollection.size()));

		// Initialize iterators for both input collections
	...
	public static void main(String[] args) {
		...
		BiFunction<Character, Integer, String> positionedLetter = (letter, position) -> position + ". " + letter;
